...
while(1){
	printf("\nselect mode\n\
	1. Enter 1 for Sender\n\
	2. Enter 2 for Receiver\n\
	Option:>");
	scanf("%d",&option);
...
msg_len = recvfrom(server_fd,(char *)buffer,1024,MSG_WAITALL, (struct sockaddr *)&address, &addrlen);
buffer[msg_len]='\0';
printf("Received: %s\n",buffer);
}
